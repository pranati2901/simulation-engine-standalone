...
import TakeCommand from '../components/TakeCommand.jsx'
  ...
  return (
    <div className="mc mc-live">
      <div className="mc-top">
        ...
        {f && selRef.current && <button className="mc-new mc-command" title="Interactive repair drill — respond to the fault live, then repair it, and get graded" onClick={() => setDrillOn(true)}>🎮 Fix It Live</button>}
      ...
      {drillOn && f && selRef.current && (
        <TakeCommand
          key={`${selRef.current.assetId}:${selRef.current.faultId}`}
          assetId={selRef.current.assetId}
          faultId={selRef.current.faultId}
          facts={f}
          conditions={f.conditions || []}
          startFrame={live}
          onExit={() => setDrillOn(false)}
        />
      )}
    </div>
  )
